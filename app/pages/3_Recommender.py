...
# Find best models for warm sessions
warm_mrr_scores = []
for model_name, key in warm_models:
    if key in warm:
        warm_mrr_scores.append((model_name, warm[key]["MRR@10"]))
best_warm_mrr_model = max(warm_mrr_scores, key=lambda x: x[1]) if warm_mrr_scores else ("N/A", 0)

warm_novelty_scores = []
for model_name, key in warm_models:
    if key in warm:
        warm_novelty_scores.append((model_name, warm[key]["Novelty@10"]))
best_warm_novelty_model = max(warm_novelty_scores, key=lambda x: x[1]) if warm_novelty_scores else ("N/A", 0)
...
"""
    ...
    warm_model_order = [name for name, _ in warm_models if _ in warm]
...
with cold_tab:
    st.markdown(
        """
For cold sessions, **Sequential warm-up** and **Item2Vec** are compared against the global popularity baseline.
"""
    )

    cold_summary_cols = st.columns(3)
    if "sequential_warmup" in cold:
    ...
    cold_model_order = [name for name, _ in cold_models if _ in cold]
